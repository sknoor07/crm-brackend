 ...
 */
export const createCustomerJob = async (req, res) => {
    try {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            for (const item of createdJobItems) {
                const updatedItem = await updateJobItemWithStatusTransition(item.id, 'created', 'pending_cs_verification', async (tx) => {
                    const [updated] = await tx
                        .update(jobItems)
                        .set({
                        ...
                        .returning();
                    return updated;
                }, customerId, 'Job item submitted and awaiting CS verification', tx);
        ...
        return res.status(500).json({
            error: 'Internal server error while creating repair request',
        });
    }
};
...
export const respondToQuote = async (req, res) => {
    try {
        const { jobId, decision, comment, } = req.body;
        ...
        if (roles.includes('customer') &&
            ...
            return res.status(403).json({
                error: 'You can only respond to quotes for your own jobs',
            });
        }
        // ---------------------------------------------------------
        ...
        if (quotedItems.length === 0) {
            return res.status(400).json({
                error: 'The quote does not contain any job items',
            });
        }
        // ---------------------------------------------------------
        // Customer decision
        ...
        await db.transaction(async (tx) => {
            ...
            if (!accepted) {
                for (const item of quotedItems) {
                    const updatedItem = await updateJobItemWithStatusTransition(item.jobItemId, item.currentStatus, item.currentStatus !== "repair_rejected" ? 'pending_final_quote' : "repair_rejected", async (transaction) => {
                        const [result] = await transaction
                            .update(jobItems)
                            .set({
                            currentStatus: item.currentStatus === "repair_rejected"
                                ? "repair_rejected"
                                : "pending_final_quote",
                            ...
                            .where(eq(jobItems.id, item.jobItemId))
                            .returning();
                        return result;
                    }, userId, comment?.trim() ||
                        'Customer rejected the final quote. Quote returned for negotiation.', tx);
            ...
            for (const item of quotedItems) {
                ...
                const isCustomerSite = item.repairLocation === 'customer_site';
                const isLab = item.repairLocation === 'inlab';
                if (!isCustomerSite && !isLab) {
                    throw new Error(`Unsupported repair location '${item.repairLocation}' for job item ${item.jobItemId}`);
                }
                if (isCustomerSite) {
                    hasOnsiteItem = true;
                }
                ...
                const updatedItem = await updateJobItemWithStatusTransition(item.jobItemId, item.currentStatus, newStatus, async (transaction) => {
                    const [result] = await transaction
                        .update(jobItems)
                        .set({
                        currentStatus: newStatus,
                        isFinalQuoteApproved: true,
                        onsiteRepairAuthorized: isCustomerSite,
                        inlabRepairAuthorized: isLab,
                        updatedAt: new Date(),
                    })
                        .where(eq(jobItems.id, item.jobItemId))
                        .returning();
                    return result;
                }, userId, comment?.trim() || itemComment, tx);
                updatedItems.push(updatedItem);
            }
            // =======================================================
            // JOB STATUS
        ...
        return res.status(500).json({
            error: 'Internal server error while responding to quote',
        });
    }
};
export const getCustomerPendingQuotes = async (req, res) => {
    try {
        const customerId = req.user?.userId;
        if (!customerId) {
            return res.status(401).json({
        ...
        const allQuotes = await db
            .select()
            .from(jobQuotes)
            .where(inArray(jobQuotes.jobId, jobIds))
            .orderBy(desc(jobQuotes.version));
        // ---------------------------------------------------------
        ...
        const quoteItems = await db
            ...
            .from(jobItemQuotes)
            .innerJoin(jobItems, eq(jobItems.id, jobItemQuotes.jobItemId))
            .where(inArray(jobItemQuotes.jobQuoteId, pendingQuoteIds));
        // ---------------------------------------------------------
        // Get quote item IDs
        // ---------------------------------------------------------
        const quoteItemIds = quoteItems.map((row) => row.quoteItem.id);
        // ---------------------------------------------------------
        // Get components
        // ---------------------------------------------------------
        const quoteLines = quoteItemIds.length > 0
            ? await db
                .select()
                .from(jobItemQuoteLines)
                .where(inArray(jobItemQuoteLines.quoteId, quoteItemIds))
                .orderBy(jobItemQuoteLines.sortOrder)
            : [];
        // ---------------------------------------------------------
        // Group components by quote item
        // ---------------------------------------------------------
        const linesByQuoteItemId = new Map();
        for (const line of quoteLines) {
            const existing = linesByQuoteItemId.get(line.quoteId);
            if (existing) {
                existing.push(line);
            }
            else {
                linesByQuoteItemId.set(line.quoteId, [line]);
            }
        }
        // ---------------------------------------------------------
        // Job lookup
        // ---------------------------------------------------------
        const jobById = new Map(customerJobs.map((job) => [
            job.id,
            job,
        ]));
        // ---------------------------------------------------------
        // Build response
        // ---------------------------------------------------------
        const result = pendingQuotes.map((quote) => {
            const job = jobById.get(quote.jobId);
            const items = quoteItems
                .filter((row) => row.quoteItem
                .jobQuoteId ===
                quote.id)
                .map(({ quoteItem, jobItem, }) => {
                const lines = linesByQuoteItemId.get(quoteItem.id) ?? [];
                return {
                    jobItemId: jobItem.id,
                    deviceCategory: jobItem.deviceCategory,
                    deviceSerialNumber: jobItem.deviceSerialNumber,
                    issueDescription: jobItem.issueDescription,
                    issueCategory: jobItem.issueCategory,
                    repairLocation: jobItem.repairLocation,
        ...
        console.error('Get customer pending quotes error:', error);
        return res.status(500).json({
            status: 'error',
