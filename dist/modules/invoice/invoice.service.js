...
export const createInvoiceForJob = async (jobId) => {
    return await db.transaction(async (tx) => {
        ...
        const [finalQuote] = await tx
            ...
            .limit(1);
        if (!finalQuote) {
            throw new Error('Final quote not found for this job');
        }
        // --------------------------------------------------
        ...
        for (const itemQuote of itemQuotes) {
            const lines = await tx
                ...
                .where(eq(jobItemQuoteLines.quoteId, itemQuote.id));
            if (lines.length === 0) {
                continue;
            }
            await tx
...
export const generateAndStoreInvoicePdf = async (invoiceId) => {
    // -----------------------------------------------
    // 1. Get invoice data
    // -----------------------------------------------
    ...
    // -----------------------------------------------
    await uploadPdf(storageKey, pdfBuffer);
    // -----------------------------------------------
    // 5. Update invoice
    // -----------------------------------------------
