...
// --------------------------------------------------
export const validateNoDuplicateJobItems = (items) => {
...
// --------------------------------------------------
export const validateJobItemsBelongToJob = (submittedItems, existingJobItems) => {
...
// --------------------------------------------------
export const validateAllJobItemsSubmitted = (existingJobItems, submittedItems) => {
...
// Get service charge
// --------------------------------------------------
const getServiceCharge = async (deviceCategory, tx) => {
    const { deviceServiceCharges } = await import('../../../db/schema/device-service-charges.js');
    const [serviceCharge] = await tx
        .select()
        .from(deviceServiceCharges)
        .where(eq(deviceServiceCharges.deviceCategory, deviceCategory))
        .limit(1);
    if (!serviceCharge) {
        throw new Error(`No service charge configured for device category '${deviceCategory}'`);
    }
    return Number(serviceCharge.chargeAmount);
};
// --------------------------------------------------
// Calculate service charge based on repair location
// --------------------------------------------------
export const calculateServiceCharge = async (jobItem, tx) => {
    // Onsite repairs have no service charge.
    if (jobItem.repairLocation ===
        'customer_site') {
        return 0;
    }
    // Lab repairs use configured device-category charge.
    return getServiceCharge(jobItem.deviceCategory, tx);
};
// --------------------------------------------------
// Approve individual job item
// --------------------------------------------------
export const approveJobItemByCS = async (tx, params) => {
    const { jobItem, input, changedBy, jobQuoteId, } = params;
    // ------------------------------------------------
    // Validate estimated components
    // ------------------------------------------------
    if (input.estimatedComponents.length === 0) {
    ...
    // ------------------------------------------------
    const serviceChargeAmount = await calculateServiceCharge(jobItem, tx);
    const totalAmount = quoteValues.componentsCost +
        serviceChargeAmount;
    // ------------------------------------------------
    // Update job item through transition helper
    // ------------------------------------------------
    const result = await updateJobItemWithStatusTransition(jobItem.id, jobItem.currentStatus, 'approved_for_transport', async (transaction) => {
        const [updatedJobItem] = await transaction
            .update(jobItems)
            .set({
    ...
    }, changedBy, input.comment.trim(), tx);
    // ------------------------------------------------
    // Create item quote
    // ------------------------------------------------
    const [quote] = await tx
        ...
        serviceCharge: moneyString(serviceChargeAmount),
        totalAmount: moneyString(totalAmount),
        createdByUserId: changedBy,
    })
        .returning();
    if (!quote) {
        throw new Error(`Failed to create estimated quote for job item ${jobItem.id}`);
    }
    // ------------------------------------------------
    // Create quote lines
    // ------------------------------------------------
    if (quoteValues.lines.length > 0) {
        await tx
            ...
            .values(quoteValues.lines.map((line) => ({
            ...line,
            quoteId: quote.id,
        })));
    }
...
// --------------------------------------------------
export const rejectJobItemByCS = async (tx, params) => {
    const { jobItem, input, changedBy, } = params;
    const updatedJobItem = await updateJobItemWithStatusTransition(jobItem.id, jobItem.currentStatus, 'repair_rejected', async (transaction) => {
        const [updated] = await transaction
            ...
            .returning();
        if (!updated) {
            throw new Error(`Failed to reject job item ${jobItem.id}`);
        }
        return updated;
    }, changedBy, input.comment.trim(), tx);
    return {
...
// --------------------------------------------------
export const processJobItemForCS = async (tx, params) => {
    const { jobItem, input, changedBy, jobQuoteId, } = params;
    if (jobItem.currentStatus !==
        'pending_cs_verification') {
        throw new Error(`Job item ${jobItem.id} is not pending CS verification`);
    }
...
// Reject entire job
// --------------------------------------------------
export const rejectJobByCS = async (tx, params) => {
    const { job, changedBy, comment, } = params;
    // ------------------------------------------------
    // Transition job
    // ------------------------------------------------
    return transitionJob({
        ...
        updateJob: async (transaction) => {
            const [updatedJob] = await transaction
                ...
                .returning();
            if (!updatedJob) {
                throw new Error(`Failed to reject job ${job.id}`);
            }
            return updatedJob;
        },
    });
...
// --------------------------------------------------
export const approveJobByCS = async (tx, params) => {
    const { job, changedBy, comment, } = params;
    // ------------------------------------------------
    // Transition job
    // ------------------------------------------------
    return transitionJob({
        ...
        updateJob: async (transaction) => {
            const [updatedJob] = await transaction
                ...
                .returning();
            if (!updatedJob) {
                throw new Error(`Failed to approve job ${job.id}`);
            }
            return updatedJob;
        },
    });
...
// --------------------------------------------------
const createJobItemInputMap = (items) => {
...
// --------------------------------------------------
export const processCSJobApproval = async (input, csUserId) => {
    return db.transaction(async (tx) => {
        ...
        if (job.currentStatus ===
            'cancelled') {
            throw new Error('Cancelled jobs cannot be reviewed by CS');
        }
        // ----------------------------------------------
        // 3. Get all job items
        // ----------------------------------------------
        const existingJobItems = await tx
            .select()
            .from(jobItems)
        ...
        // ----------------------------------------------
        const hasApprovedItem = input.items.some((item) => item.decision ===
            'approved');
        // ----------------------------------------------
        // 7. Get next job quote version
        // ----------------------------------------------
        const [latestJobQuote] = await tx
            .select()
            .from(jobQuotes)
            .where(eq(jobQuotes.jobId, job.id))
            .orderBy(desc(jobQuotes.version))
            .limit(1);
        const nextJobQuoteVersion = (latestJobQuote?.version ?? 0) + 1;
        // ----------------------------------------------
        ...
        const [jobQuote] = await tx
            ...
            .returning();
        if (!jobQuote) {
            throw new Error(`Failed to create job quote for job ${job.id}`);
        }
        // ----------------------------------------------
        ...
        for (const jobItem of existingJobItems) {
            ...
            const result = await processJobItemForCS(tx, {
                jobItem,
                input: itemInput,
                changedBy: csUserId,
        ...
        // ----------------------------------------------
        const [updatedJobQuote] = await tx
            .update(jobQuotes)
            .set({
            ...
            .returning();
        if (!updatedJobQuote) {
            throw new Error(`Failed to finalize job quote ${jobQuote.id}`);
        }
        // ----------------------------------------------
        // 12. All items rejected
        // ----------------------------------------------
        if (!hasApprovedItem) {
            const updatedJob = await rejectJobByCS(tx, {
                job,
                changedBy: csUserId,
        ...
        // ----------------------------------------------
        const updatedJob = await approveJobByCS(tx, {
            job,
            changedBy: csUserId,
            comment: input.comment,
        });
