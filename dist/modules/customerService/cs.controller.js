...
import { jobs, jobItems, jobComments, jobStatusHistory, jobClosures, jobItemQuotes, jobItemQuoteLines, jobItemStatusHistory, users, customerProfiles, jobQuotes, invoices, warranties, } from '../../db/schema/index.js';
...
const createWarrantiesForClosedJob = async (tx, jobId, warrantyStart) => {
    // --------------------------------------------------
    // Get latest final quote
    // --------------------------------------------------
    const [finalQuote] = await tx
        .select()
        .from(jobQuotes)
        .where(and(eq(jobQuotes.jobId, jobId), eq(jobQuotes.status, "final")))
    ...
    if (!finalQuote) {
        const deliveredItems = await tx
            ...
            .from(jobItems)
            .where(and(eq(jobItems.jobId, jobId), eq(jobItems.currentStatus, "delivered")));
        if (deliveredItems.length > 0) {
            throw new Error("Cannot create warranties because no final quote was found.");
        }
        return;
    }
    // --------------------------------------------------
    ...
    const quoteItems = await tx
        .select({
        itemQuote: jobItemQuotes,
        item: jobItems,
    })
        .from(jobItemQuotes)
        .innerJoin(jobItems, eq(jobItems.id, jobItemQuotes.jobItemId))
        .where(eq(jobItemQuotes.jobQuoteId, finalQuote.id));
    // --------------------------------------------------
    // Get quote lines
    // --------------------------------------------------
    for (const quoteItem of quoteItems) {
        // Only delivered items can receive warranty.
        if (quoteItem.item.currentStatus !==
            "delivered") {
            continue;
        }
        const lines = await tx
            ...
            .where(eq(jobItemQuoteLines.quoteId, quoteItem.itemQuote.id));
        for (const line of lines) {
            // 0 = no warranty
            if (line.warrantyMonths <= 0) {
                continue;
            }
            const warrantyEnd = addMonths(warrantyStart, line.warrantyMonths);
            await tx
                .insert(warranties)
                .values({
                jobId,
                jobItemId: quoteItem.item.id,
                quoteLineId: line.id,
                componentName: line.name,
                quantity: line.quantity,
                deviceSerialNumber: quoteItem.item.deviceSerialNumber,
                deviceImei: null,
...
export const seacrhCustomerDetailswithJob = async (req, res) => {
    try {
...
export const searchCustomers = async (req, res) => {
    try {
        ...
        const customers = await db
            ...
            .leftJoin(jobs, eq(jobs.customerId, users.id))
            .where(or(ilike(customerProfiles.firstName, `%${search}%`), ilike(customerProfiles.lastName, `%${search}%`), ilike(users.email, `%${search}%`), ilike(customerProfiles.phone, `%${search}%`), ilike(jobs.jobNumber, `%${search}%`)))
            .groupBy(users.id, customerProfiles.firstName, customerProfiles.lastName, users.email, customerProfiles.phone);
...
export const getAllCustomers = async (req, res) => {
    try {
        const customers = await db
            ...
            .innerJoin(customerProfiles, eq(customerProfiles.userId, users.id))
            .leftJoin(jobs, eq(jobs.customerId, users.id))
            .groupBy(users.id, customerProfiles.firstName, customerProfiles.lastName, users.email, customerProfiles.phone).orderBy(desc(users.createdAt));
        return res.status(200).json({
            status: 'success',
            data: customers,
        });
 ...
 */
export const getJobsWaitingToBeClosed = async (req, res) => {
    try {
        ...
        const hasNonFinalItems = db.select({ id: jobItems.id, })
            .from(jobItems)
            .where(and(eq(jobItems.jobId, jobs.id), sql `${jobItems.currentStatus} NOT IN ('delivered', 'repair_rejected','cancelled','removed_from_quote')`));
        /**
...
// --------------------------------------------------
export const closeJobRequest = async (req, res) => {
    try {
        // --------------------------------------------------
        // AUTHENTICATION
        // --------------------------------------------------
        ...
        // REQUEST BODY
        // --------------------------------------------------
        const { jobId, customerConfirmed, paymentConfirmed, closureReason, } = req.body;
        // --------------------------------------------------
        // VALIDATION 1
        ...
        // delivered -> closed
        // --------------------------------------------------
        if (!canTransitionJob(job.currentStatus, 'closed')) {
            return res.status(400).json({
                status: 'error',
                error: 'Job cannot be closed.',
                message: `Job cannot transition from ` +
        ...
        if (nonFinalItems.length > 0) {
            return res.status(400).json({
                status: 'error',
                error: 'Job cannot be closed.',
                message: 'All job items must be either delivered or repair_rejected.',
        ...
        const result = await db.transaction(async (tx) => {
            ...
            const [currentJob] = await tx
                .select()
                .from(jobs)
                .where(and(eq(jobs.id, jobId), or(eq(jobs.currentStatus, 'delivered'), eq(jobs.currentStatus, 'repair_completed'))))
                .limit(1);
            if (!currentJob) {
                throw new Error('Job is no longer in delivered status.');
            }
            // --------------------------------------------------
            // Re-check transition
            // --------------------------------------------------
            if (!canTransitionJob(currentJob.currentStatus, 'closed')) {
                throw new Error(`Job cannot transition from ` +
            ...
            const [existingClosure] = await tx
                ...
                .limit(1);
            if (existingClosure) {
                throw new Error('Job has already been closed.');
            }
            // --------------------------------------------------
            // JOB TRANSITION
            //
            ...
            const updatedJob = await transitionJob({
                ...
                updateJob: (tx) => tx
                    ...
                    .where(and(eq(jobs.id, jobId), eq(jobs.currentStatus, currentJob.currentStatus)))
                    .returning(),
            });
            // --------------------------------------------------
            // JOB CLOSURE
            ...
            const [closure] = await tx
                ...
                .returning();
            await createWarrantiesForClosedJob(tx, jobId, closedAt);
            return {
                job: updatedJob,
                closure,
            };
        ...
        // --------------------------------------------------
        if (error instanceof Error &&
            error.message.startsWith('Invalid job status transition:')) {
            return res.status(400).json({
                status: 'error',
                error: error.message,
            });
...
export const approveJobByCS = async (req, res) => {
    try {
        const csUserId = req.user?.userId;
        if (!csUserId) {
            return res.status(401).json({
                status: 'error',
                message: 'Authenticated CS user is required',
            });
        }
...
export const getPendingFinalQuotes = async (req, res) => {
    try {
        // --------------------------------------------------
        ...
        const jobIds = jobsPendingQuote.map(({ job }) => job.id);
        // --------------------------------------------------
        ...
        const pendingQuoteItems = await db
            .select()
            .from(jobItems)
            .where(and(inArray(jobItems.jobId, jobIds), or(eq(jobItems.currentStatus, "pending_final_quote"), eq(jobItems.currentStatus, "repair_rejected"), eq(jobItems.currentStatus, "cancelled"))))
            .orderBy(desc(jobItems.updatedAt));
        // --------------------------------------------------
        // 3. Only jobs having at least one
        //    pending_final_quote item should appear
        // --------------------------------------------------
        const jobsWithPendingItems = new Set(pendingQuoteItems
            .filter((item) => item.currentStatus ===
            "pending_final_quote")
            .map((item) => item.jobId));
        const filteredJobs = jobsPendingQuote.filter(({ job }) => jobsWithPendingItems.has(job.id));
        ...
        const itemIds = pendingQuoteItems.map((item) => item.id);
        const comments = itemIds.length > 0
            ? await db
                .select({
                id: jobComments.id,
                ...
                .from(jobComments)
                .where(or(inArray(jobComments.jobId, filteredJobIds), inArray(jobComments.jobItemId, itemIds)))
                .orderBy(desc(jobComments.createdAt), desc(jobComments.id))
            : [];
        // --------------------------------------------------
        ...
        for (const comment of comments) {
            if (comment.jobId &&
                ...
                latestJobComments.set(comment.jobId, comment);
            }
            if (comment.jobItemId &&
                !latestItemComments.has(comment.jobItemId)) {
                latestItemComments.set(comment.jobItemId, comment);
            }
        }
        // --------------------------------------------------
        // 7. Helper for latest comment
        // --------------------------------------------------
        const toLatestComment = (comment) => comment
            ? {
                id: comment.id,
        ...
        // 9. Build response
        // --------------------------------------------------
        const result = filteredJobs.map(({ job, customer }) => {
            const items = itemsByJobId.get(job.id) ?? [];
            return {
                job: {
                    ...job,
                    latestComment: toLatestComment(latestJobComments.get(job.id)),
                },
                customer,
                items: items.map((item) => ({
                    ...item,
                    latestComment: toLatestComment(latestItemComments.get(item.id)),
                })),
            };
        });
        // --------------------------------------------------
        ...
        console.error("Error fetching jobs pending final quote:", error);
        return res.status(500).json({
            status: "error",
            message: "Failed to fetch jobs pending final quote",
        });
    }
};
export const getQuoteForAJob = async (req, res) => {
    try {
        ...
        const comments = await db
            ...
            ? or(eq(jobComments.jobId, jobId), inArray(jobComments.jobItemId, itemIds))
            : eq(jobComments.jobId, jobId))
            .orderBy(desc(jobComments.createdAt), desc(jobComments.id));
        // --------------------------------------------------
        // 5. Latest job-level comment
        // --------------------------------------------------
        const latestJobComment = comments.find((comment) => comment.jobId === jobId &&
            comment.jobItemId === null) ?? null;
        // --------------------------------------------------
        ...
        if (!latestQuote) {
            return res.status(200).json({
                status: "success",
                quote: null,
                latestComment: latestJobComment
                    ? {
                        id: latestJobComment.id,
                        userId: latestJobComment.userId,
                        comment: latestJobComment.comment,
                        createdAt: latestJobComment.createdAt,
                    }
                    : null,
                items: items.map((item) => ({
                    jobItemId: item.id,
                    quote: null,
                    lines: [],
                    latestComment: latestItemComments.has(item.id)
                        ? {
                            id: latestItemComments.get(item.id).id,
                            userId: latestItemComments.get(item.id).userId,
                            comment: latestItemComments.get(item.id).comment,
                            createdAt: latestItemComments.get(item.id).createdAt,
                        }
                        : null,
                })),
            });
        ...
        const itemQuotes = await db
            .select()
            .from(jobItemQuotes)
            .where(eq(jobItemQuotes.jobQuoteId, latestQuote.id));
        // --------------------------------------------------
        // 9. Get quote lines
        // --------------------------------------------------
        const itemQuoteIds = itemQuotes.map((itemQuote) => itemQuote.id);
        const quoteLines = itemQuoteIds.length > 0
            ? await db
                .select()
                .from(jobItemQuoteLines)
                .where(inArray(jobItemQuoteLines.quoteId, itemQuoteIds))
            : [];
        // --------------------------------------------------
        ...
        // --------------------------------------------------
        const itemQuoteByItemId = new Map(itemQuotes.map((itemQuote) => [
            itemQuote.jobItemId,
            itemQuote,
        ]));
        // --------------------------------------------------
        ...
        const result = {
            quote: latestQuote,
            latestComment: toLatestComment(latestJobComment),
            items: items.map((item) => {
                const itemQuote = itemQuoteByItemId.get(item.id);
                return {
                    jobItemId: item.id,
                    quote: itemQuote
                        ? {
                        ...
                        : null,
                    lines: itemQuote
                        ? linesByQuoteId.get(itemQuote.id) ?? []
                        : [],
                    latestComment: toLatestComment(latestItemComments.get(item.id)),
                };
            }),
...
export const generateFinalQuote = async (req, res) => {
    try {
        const csUserId = req.user?.userId;
        if (!csUserId) {
            return res.status(401).json({
        ...
        const result = await db.transaction(async (tx) => {
            ...
            if (job.currentStatus !== "pending_final_quote") {
                throw new Error(`Final quote can only be generated when job status is pending_final_quote. Current status: ${job.currentStatus}`);
            }
            // ---------------------------------------------------------
            // 3. Get ALL job items
            // ---------------------------------------------------------
            ...
            if (existingItems.length === 0) {
                throw new Error("Cannot generate final quote because the job has no items");
            }
            // ---------------------------------------------------------
            ...
            const invalidItems = existingItems.filter((item) => !allowedStatuses.has(item.currentStatus));
            if (invalidItems.length > 0) {
                throw new Error("Final quote cannot be generated because some job items are not ready. " +
            ...
            if (items.length !== existingItems.length) {
                throw new Error("All existing job items must be included in the final quote");
            }
            // ---------------------------------------------------------
            // 6. Create lookup map
            // ---------------------------------------------------------
            ...
            for (const inputItem of items) {
                ...
                if (!existingItem) {
                    throw new Error(`Job item ${inputItem.jobItemId} does not belong to this job`);
                }
                if (submittedItemIds.has(inputItem.jobItemId)) {
                    throw new Error(`Job item ${inputItem.jobItemId} was submitted more than once`);
                }
            ...
            for (const existingItem of existingItems) {
                if (!submittedItemIds.has(existingItem.id)) {
                    throw new Error(`Job item ${existingItem.id} is missing from the final quote`);
                }
            }
            // ---------------------------------------------------------
            ...
            for (const inputItem of items) {
                const existingItem = existingItemMap.get(inputItem.jobItemId);
                const isPendingFinalQuote = existingItem.currentStatus === "pending_final_quote";
                const isRejectedOrCancelled = existingItem.currentStatus === "repair_rejected" ||
                ...
                if (components === undefined) {
                    ...
                    if (previousItemQuote) {
                        const previousLines = await tx
                            .select()
                            .from(jobItemQuoteLines)
                            .where(eq(jobItemQuoteLines.quoteId, previousItemQuote.id))
                ...
                if (isPendingFinalQuote) {
                    ...
                    serviceCharge = await calculateServiceCharge(existingItem, tx);
                }
                ...
                // -------------------------------------------------------
                const quoteResult = await insertJobItemQuoteWithLines(tx, {
                ...
                // -------------------------------------------------------
                if (isPendingFinalQuote) {
                    await updateJobItemWithStatusTransition(existingItem.id, "pending_final_quote", "awaiting_customer_approval", async (transaction) => {
                        const [updatedItem] = await transaction
                            .update(jobItems)
                            .set({
                            currentStatus: "awaiting_customer_approval",
                            finalComponentsCost: quoteResult.componentsCost.toFixed(2),
                            serviceChargeApplied: quoteResult.serviceChargeAmount.toFixed(2),
                    ...
                    "Final quote generated and sent for customer approval.", tx);
                }
                else {
                    // -----------------------------------------------------
