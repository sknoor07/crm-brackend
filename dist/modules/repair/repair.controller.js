...
import { jobs, jobItems, roles, userRoles, users, } from '../../db/schema/index.js';
 ...
 */
const isActiveRepairPerson = async (userId) => {
    const [repairPerson] = await db
        ...
        .where(and(eq(users.id, userId), eq(users.userType, 'employee'), eq(users.isActive, true), eq(roles.name, 'repair_person')))
        .limit(1);
 ...
 */
const isRepairPersonAuthorized = (item, userId, userRolesList = []) => {
    if (userRolesList.includes('admin')) {
        return true;
    }
    return (item.assignedRepairPersonId === userId);
};
/**
 ...
 */
const isRepairManagerAuthorized = (job, userId, userRolesList = []) => {
    if (userRolesList.includes('admin')) {
        return true;
    }
    return (job.repairManagerId === userId &&
 ...
 */
const getJobItemWithJob = async (jobItemId) => {
 ...
 *       assignment
 */
export const getPendingRepairAssignments = async (req, res) => {
    try {
        ...
        const ismanager = req.user?.roles?.includes('repair_manager');
        ...
        console.error('Fetch pending repair assignments error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
};
export const getAllRepairPersons = async (req, res) => {
    try {
 ...
 */
export const assignRepairPerson = async (req, res) => {
    try {
        const { jobItemId, repairPersonId, comment, } = req.body;
        const managerId = req.user?.userId;
        if (!managerId) {
            return res.status(401).json({
                error: 'Authenticated user not found',
        ...
        const result = await getJobItemWithJob(jobItemId);
        if (!result) {
         ...
         */
        if (!isRepairManagerAuthorized(job, managerId, req.user?.roles)) {
            return res.status(403).json({
                error: 'You are not authorized to assign repair items for this job.',
            });
        }
        /*
        ...
        if (item.currentStatus !==
            ...
            return res.status(400).json({
                error: `Item must be 'assigned_to_repair_manager' ` +
         ...
         */
        if (!(await isActiveRepairPerson(repairPersonId))) {
            return res.status(400).json({
                error: 'Assigned user must be an active employee with the repair_person role.',
            });
        }
        /*
         ...
         */
        const updatedItem = await updateJobItemWithStatusTransition(item.id, item.currentStatus, 'assigned_to_repair_person', async (tx) => {
            const [updated] = await tx
                .update(jobItems)
                .set({
                assignedRepairPersonId: repairPersonId,
        ...
        console.error('Assign repair person error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
 ...
 */
export const getAssignedRepairItems = async (req, res) => {
    try {
        ...
        const items = await db
            ...
            .from(jobItems)
            .innerJoin(jobs, eq(jobItems.jobId, jobs.id))
            .where(and(eq(jobItems.assignedRepairPersonId, userId), eq(jobItems.currentStatus, 'assigned_to_repair_person')))
            .orderBy(desc(jobItems.updatedAt));
        return res.status(200).json({
            count: items.length,
            items,
        ...
        console.error('Fetch assigned repair items error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
 ...
 */
////////////////////////////////////////////////////////////////done ///////////////////////////////////////////////////////
export const requestFinalQuote = async (req, res) => {
    try {
        ...
        const [existingItem] = await db
            .select()
            .from(jobItems)
            .where(and(eq(jobItems.id, jobItemId), eq(jobItems.assignedRepairPersonId, repairPersonId)))
            .limit(1);
        if (!existingItem) {
            return res.status(400).json({
        ...
        const [job] = await db
            .select()
            .from(jobs)
            .where(and(eq(jobs.id, existingItem.jobId), eq(jobs.currentStatus, 'repair_in_progress')))
            .limit(1);
        if (!job) {
            return res.status(404).json({
        ...
        if (job.currentStatus !== 'repair_in_progress') {
            return res.status(400).json({
                error: `In-lab inspection cannot be completed when ` +
        ...
        // --------------------------------------------------
        if (existingItem.currentStatus !==
            'assigned_to_repair_person') {
            return res.status(400).json({
                error: `Item with status ${existingItem.currentStatus} ` +
                    `cannot start repair.`,
            });
        }
        // --------------------------------------------------
        ...
        if (decision !== 'repairable' &&
            ...
            return res.status(400).json({
                error: 'Invalid inspection decision. Decision must be repairable or unrepairable.',
            });
        }
        // --------------------------------------------------
        // Job note
        ...
        const result = await db.transaction(async (tx) => {
            ...
            // --------------------------------------------
            let newJobStatus;
            const canMoveToFinalQuote = allItems.every((item) => item.currentStatus === 'pending_final_quote' ||
                item.currentStatus === 'repair_rejected');
            const hasUnrepairableItem = allItems.some((item) => item.currentStatus ===
                'assigned_to_repair_manager');
        ...
        console.error('Request final quote error:', error);
        return res.status(500).json({
 ...
 */
export const getPendingRepairInspections = async (req, res) => {
    try {
        const managerId = req.user?.userId;
        if (!managerId) {
            return res.status(401).json({
        ...
        const items = await db
            ...
            .innerJoin(jobs, eq(jobItems.jobId, jobs.id))
            .where(isAdmin
            ? eq(jobItems.currentStatus, 'pending_repair_manager_inspection')
            : and(eq(jobItems.currentStatus, 'pending_repair_manager_inspection'), eq(jobs.repairManagerId, managerId)))
            .orderBy(desc(jobItems.updatedAt));
        return res.status(200).json({
            count: items.length,
            items,
        ...
        console.error('Fetch pending repair inspections error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
...
export const approveRepair = async (req, res) => {
    try {
        ...
        if (!isRepairManagerAuthorized(job, managerId, req.user?.roles)) {
            return res.status(403).json({
                error: 'You are not authorized to inspect this repair item.',
            });
        }
        if (item.currentStatus !==
            'pending_repair_manager_inspection') {
            return res.status(400).json({
                error: `Cannot approve item from '${item.currentStatus}'.`,
            });
        }
        const updatedItem = await updateJobItemWithStatusTransition(jobItemId, item.currentStatus, 'ready_for_delivery', async (tx) => {
            const [updated] = await tx
                .update(jobItems)
                .set({
                updatedAt: new Date(),
        ...
        return res.status(200).json({
            message: 'Repair approved by Repair Team Manager.',
            item: updatedItem,
        });
    }
    catch (error) {
