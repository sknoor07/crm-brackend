...
export const createOtp = async ({ userId, purpose, channel, destination, }) => {
    ...
    await db
        ...
        .where(and(eq(otpVerifications.userId, userId), eq(otpVerifications.purpose, purpose), isNull(otpVerifications.verifiedAt)));
    // 2. Generate OTP
...
export const verifyOtp = async ({ userId, purpose, otp, }) => {
    ...
    const [record] = await db
        ...
        .where(and(eq(otpVerifications.userId, userId), eq(otpVerifications.purpose, purpose), isNull(otpVerifications.verifiedAt), gt(otpVerifications.expiresAt, now)))
