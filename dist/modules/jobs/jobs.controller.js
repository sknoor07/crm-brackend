...
import { users, customerProfiles, jobs, jobItems, jobComments, jobStatusHistory, jobItemStatusHistory, deviceServiceCharges, jobQuotes, jobItemQuotes, jobItemQuoteLines, invoices, invoiceItems, jobClosures, employeeProfiles, } from '../../db/schema/index.js';
...
export const getJobWithItems = async (req, res) => {
    try {
        ...
        const searchCondition = search
            ? or(ilike(jobs.jobNumber, `%${search}%`), ilike(users.email, `%${search}%`), ilike(users.phone, `%${search}%`), ilike(customerProfiles.firstName, `%${search}%`), ilike(customerProfiles.lastName, `%${search}%`)) : undefined;
...
export const getJobDetails = async (req, res) => {
    try {
        ...
        const statusHistory = await db
            ...
            .leftJoin(users, eq(users.id, jobStatusHistory.changedBy)).leftJoin(employeeProfiles, eq(employeeProfiles.userId, users.id))
            .leftJoin(customerProfiles, eq(customerProfiles.userId, users.id))
            .where(eq(jobStatusHistory.jobId, jobRow.job.id))
            .orderBy(asc(jobStatusHistory.createdAt));
        const formattedStatusHistory = statusHistory.map((history) => {
            const isEmployee = history.changedByUserType === "employee";
            ...
            return {
                ...
                changedBy: history.changedById
                    ? {
                        ...
                        userType: history.changedByUserType,
                    }
                    : null,
            };
        });
        ...
        const itemHistory = itemIds.length > 0
            ? await db
                .select({
                ...
                .where(inArray(jobItemStatusHistory.jobItemId, itemIds))
                .orderBy(asc(jobItemStatusHistory.createdAt))
            : [];
        // --------------------------------------------------
        ...
        for (const history of itemHistory) {
            const existing = itemHistoryMap.get(history.jobItemId);
            if (existing) {
                existing.push(history);
            }
            else {
                itemHistoryMap.set(history.jobItemId, [history]);
            }
        }
        const comments = await db
            ...
            .leftJoin(customerProfiles, eq(customerProfiles.userId, users.id))
            .where(or(eq(jobComments.jobId, jobRow.job.id), itemIds.length > 0
        ...
        const itemQuotes = quoteIds.length > 0
            ? await db
                .select()
                .from(jobItemQuotes)
                .where(inArray(jobItemQuotes.jobQuoteId, quoteIds))
            : [];
        ...
        for (const line of quoteLines) {
            const existing = quoteLinesMap.get(line.quoteId);
            if (existing) {
                existing.push(line);
            }
            else {
        ...
        for (const itemQuote of itemQuotes) {
            const existing = itemQuotesMap.get(itemQuote.jobQuoteId);
            if (existing) {
                existing.push(itemQuote);
            }
            else {
                itemQuotesMap.set(itemQuote.jobQuoteId, [itemQuote]);
            }
        }
        const quotesWithItems = quotes.map((quote) => ({
            quote,
            items: (itemQuotesMap.get(quote.id) ?? []).map((itemQuote) => ({
                itemQuote,
                lines: quoteLinesMap.get(itemQuote.id) ?? [],
            })),
        }));
        ///invoice
        // --------------------------------------------------
        ...
        const [closure] = await db
            .select()
            .from(jobClosures)
            .where(eq(jobClosures.jobId, jobRow.job.id))
            .limit(1);
        // --------------------------------------------------
        ...
        const result = {
            ...
            customer: {
                ...
                firstName: jobRow.customerProfile?.firstName ?? '',
                lastName: jobRow.customerProfile?.lastName ?? '',
                email: jobRow.customerUser?.email ?? '',
                phone: jobRow.customerUser?.phone ?? '',
                billingAddress: jobRow.customerProfile?.billingAddress ?? '',
                gstin: jobRow.customerProfile?.gstin ?? '',
            },
            statusHistory: formattedStatusHistory,
            items: items.map((item) => ({
                item,
                statusHistory: itemHistoryMap.get(item.id) ?? [],
            })),
