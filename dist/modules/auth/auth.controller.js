...
import { generateAccessToken, verifyRefreshToken } from '../../shared/utils/jwt.js';
...
export const registerEmployee = async (req, res) => {
    try {
        const { email, firstName, lastName, phone, dateOfBirth, aadhaarNumber, panNumber, uanNumber, currentAddress, permanentAddress, bankAccountNumber, bankIfscCode, bankName, emergencyContactName, emergencyContactPhone, specializations, roleIds, } = req.body;
        ...
        const result = await db.transaction(async (tx) => {
            ...
            await tx.insert(userRoles)
                .values(roleIds.map((roleId) => ({ userId: newUser.id, roleId, assignedBy: req.user.userId })));
            // Create invitation
...
export const refreshAccessToken = async (req, res) => {
    try {
        ...
        const [session] = await db
            ...
            .from(refreshTokens)
            .where(and(eq(refreshTokens.userId, userId), eq(refreshTokens.tokenHash, tokenHash), eq(refreshTokens.isRevoked, false), gt(refreshTokens.expiresAt, now)))
            .limit(1);
        if (!session) {
    ...
    try {
        ...
        const user = await db.transaction(async (tx) => {
            const [invitation] = await tx.select()
                ...
                .where(and(eq(accountInvitations.tokenHash, tokenHash), isNull(accountInvitations.usedAt), gt(accountInvitations.expiresAt, now)))
                .limit(1);
            if (!invitation) {
                return null;
            }
