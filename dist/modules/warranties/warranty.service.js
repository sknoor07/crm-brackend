...
import { users, customerProfiles, } from "../../db/schema/users.js";
...
export const getWarrantyById = async (warrantyId) => {
    ...
    const claimItems = claimIds.length > 0
        ? await db
            .select()
    ...
    const repairJobIds = claims
        ...
        .filter((id) => id !== null);
    const repairJobs = repairJobIds.length > 0
        ? await db
            .select()
            .from(jobs)
    ...
    const claimsWithDetails = claims.map((claim) => {
        const repairJob = claim.repairJobId
            ? repairJobs.find((job) => job.id === claim.repairJobId) ?? null
            : null;
        const items = claimItems.filter((item) => item.warrantyClaimId === claim.id);
        return {
            ...claim,
            repairJob,
            items,
    ...
    // --------------------------------------------------
    const effectiveWarrantyStatus = getWarrantyStatus(warranty.warrantyEnd, warranty.warrantyStatus);
    // --------------------------------------------------
    // 12. Claim summary
    // --------------------------------------------------
    const totalClaims = claimsWithDetails.length;
    const openClaims = claimsWithDetails.filter((claim) => ![
        "completed",
        "rejected",
        "cancelled",
    ].includes(claim.status)).length;
    const completedClaims = claimsWithDetails.filter((claim) => claim.status === "completed").length;
    // --------------------------------------------------
    // 13. Final response
    // --------------------------------------------------
    return {
...
export const getWarranties = async (filters) => {
    const { status, customerId, jobId, jobItemId, deviceSerialNumber, componentName, page, limit, } = filters;
    ...
    if (jobId) {
        conditions.push(eq(warranties.jobId, jobId));
    }
    if (jobItemId) {
        conditions.push(eq(warranties.jobItemId, jobItemId));
    }
    if (deviceSerialNumber) {
        conditions.push(ilike(warranties.deviceSerialNumber, `%${deviceSerialNumber}%`));
    }
    if (componentName) {
        conditions.push(ilike(warranties.componentName, `%${componentName}%`));
    }
    if (status === "active") {
        conditions.push(and(eq(warranties.warrantyStatus, "active"), gte(warranties.warrantyEnd, new Date())));
    }
    if (status === "expired") {
        conditions.push(and(eq(warranties.warrantyStatus, "active"), lt(warranties.warrantyEnd, new Date())));
    }
    if (status === "void") {
        conditions.push(eq(warranties.warrantyStatus, "void"));
    }
    if (status === "completed") {
        conditions.push(eq(warranties.warrantyStatus, "completed"));
    }
    /*
    ...
    let items = rows.map((row) => ({
        ...
        deviceCategory: row.jobItem.deviceCategory,
        deviceSerialNumber: row.warranty.deviceSerialNumber,
        deviceImei: row.warranty.deviceImei,
        componentName: row.warranty.componentName,
        quantity: row.warranty.quantity,
        warrantyMonths: row.warranty.warrantyMonths,
        warrantyStart: row.warranty.warrantyStart,
        warrantyEnd: row.warranty.warrantyEnd,
        warrantyStatus: getWarrantyStatus(row.warranty.warrantyEnd, row.warranty.warrantyStatus),
        customer: {
            id: row.user.id,
        ...
        createdAt: row.warranty.createdAt,
        updatedAt: row.warranty.updatedAt,
    }));
    return {
        items,
        pagination: {
...
export const createWarrantyClaim = async (warrantyId, input) => {
    // 1. Find warranty
    const [warranty] = await db
        .select()
        .from(warranties)
    ...
    const effectiveStatus = getWarrantyStatus(warranty.warrantyEnd, warranty.warrantyStatus);
    ...
    const recentClaims = await db
        .select()
        .from(warrantyClaims)
        .where(and(eq(warrantyClaims.warrantyId, warrantyId), gte(warrantyClaims.reportedAt, thirtyDaysAgo)))
        .orderBy(desc(warrantyClaims.reportedAt));
    // Existing claims + the new claim
    const totalRecentClaims = recentClaims.length + 1;
    const repeatRepairLevel = getRepeatRepairLevel(totalRecentClaims);
    ...
    const [{ total }] = await db
        ...
        .from(warrantyClaims);
    const claimNumber = generateWarrantyClaimNumber(Number(total) + 1);
    // 5. Create claim
...
export const updateWarrantyClaimInspection = async (claimId, input) => {
    // 1. Find claim
    const [claim] = await db
        .select()
        .from(warrantyClaims)
