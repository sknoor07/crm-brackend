...
export const getWarrantyList = async (req, res) => {
    try {
        const filters = req.query;
        const result = await getWarranties({
            status: typeof filters.status === "string"
                ? filters.status
                : undefined,
            customerId: typeof filters.customerId === "string"
                ? filters.customerId
                : undefined,
            jobId: typeof filters.jobId === "string"
                ? filters.jobId
                : undefined,
            jobItemId: typeof filters.jobItemId === "string"
                ? filters.jobItemId
                : undefined,
            deviceSerialNumber: typeof filters.deviceSerialNumber ===
                "string"
                ? filters.deviceSerialNumber
                : undefined,
            componentName: typeof filters.componentName === "string"
                ? filters.componentName
                : undefined,
            page: typeof filters.page === "string"
                ? Number(filters.page)
                : 1,
            limit: typeof filters.limit === "string"
...
export const createClaim = async (req, res) => {
    try {
        ...
        const result = await createWarrantyClaim(warrantyId, parsed.data);
...
export const updateClaimInspection = async (req, res) => {
    try {
        const { claimId } = req.params;
        // 1. Validate body
        const parsed = updateWarrantyClaimInspectionSchema.safeParse(req.body);
        if (!parsed.success) {
            return res.status(400).json({
                status: "error",
                error: "Validation failed",
        ...
        // 2. Update inspection
        const claim = await updateWarrantyClaimInspection(claimId, parsed.data);
        return res.status(200).json({
            status: "success",
            data: {
                claim,
        ...
        if (error instanceof Error) {
            ...
            if (error.message === "CLAIM_NOT_OPEN") {
                return res.status(400).json({
                    status: "error",
                    error: "Only an open claim can be inspected",
                });
            }
        }
        console.error("Update warranty claim inspection error:", error);
        return res.status(500).json({
            status: "error",
            error: "Internal server error",
        });
