...
export const getDashboardSummary = async (req, res) => {
    try {
        ...
        const completed = (counts.repair_completed ?? 0) +
            (counts.delivered ?? 0) +
            (counts.closed ?? 0);
        const readyForDelivery = (counts.out_for_delivery ?? 0) +
            (counts.delivered ?? 0);
        return res.status(200).json({
            success: true,
            data: {
                queue: {
                    pendingCsVerification: counts.pending_cs_verification ?? 0,
                    pendingTransportAssignment: (counts.ready_for_pickup ?? 0) +
                ...
                workflowHealth: {
                    initialVerificationPercent: total
                        ? Math.round((((counts.pending_cs_verification ?? 0) === 0
                            ? total
                    ...
                    transportHandoffPercent: total
                        ? Math.round((((counts.in_transit_to_lab ?? 0) +
