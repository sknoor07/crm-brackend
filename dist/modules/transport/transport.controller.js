 ...
 */
const isActiveTransportPerson = async (userId) => {
 ...
 */
const isActiveDeliveryPerson = async (userId) => {
 ...
 */
const recordItemStatusChange = async (tx, jobItemId, previousStatus, newStatus, changedBy, note) => {
    await tx
 ...
 */
const addJobComment = async (tx, jobId, userId, comment) => {
    await tx.insert(jobComments).values({ jobId, userId, comment, });
};
/**
 * Add an item-level comment.
 */
const addItemComment = async (tx, jobItemId, userId, comment) => {
    await tx
        .insert(jobComments)
        .values({
        jobItemId,
...
export const getPendingPickups = async (req, res) => {
    try {
        ...
        const rawResults = await db.select({
            ...
            .where(and(eq(jobs.currentStatus, 'assigning_pickup_Engineer'), eq(jobItems.currentStatus, 'approved_for_transport')))
 ...
 */
export const assignTransportPerson = async (req, res) => {
    try {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            for (const item of itemsWaitingForTransport) {
                const updatedItem = await updateJobItemWithStatusTransition(item.id, item.currentStatus, 'transport_visit_in_progress', async (tx) => {
                    ...
                    return updated;
                }, transportManagerId, comment.trim(), tx);
...
export const getPickUpPersonList = async (req, res) => {
    try {
        const pickupPersonDetails = await db
            ...
            fullName: sql `${employeeProfiles.firstName} || ' ' || ${employeeProfiles.lastName}`,
            ...
            .innerJoin(roles, eq(userRoles.roleId, roles.id))
            .where(and(eq(users.userType, 'employee'), eq(roles.name, 'transport_team_person')));
...
export const getJobsWaitingLabReceipt = async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
...
export const receiveAtLab = async (req, res) => {
    try {
        const { jobItemId, jobItemIds, comment, } = req.body;
        const transportManagerId = req.user?.userId;
        if (!transportManagerId) {
        ...
        const hasBatchItems = Array.isArray(jobItemIds) &&
            jobItemIds.length > 0;
        if (hasSingleItem && hasBatchItems) {
            return res.status(400).json({
                error: 'Provide either jobItemId or jobItemIds, not both.',
            });
        }
        if (!hasSingleItem && !hasBatchItems) {
            return res.status(400).json({
                error: 'Either jobItemId or jobItemIds is required.',
            });
        }
        /* -----------------------------------------------------
           Normalize IDs
        ...
        /* -----------------------------------------------------
           ...
           ----------------------------------------------------- */
        const jobIds = new Set(existingItems.map((item) => item.jobId));
        if (jobIds.size !== 1) {
            return res.status(400).json({
                error: 'All job items must belong to the same job.',
            });
        }
        const jobId = existingItems[0].jobId;
        /* -----------------------------------------------------
           Find job
           ----------------------------------------------------- */
        const [job] = await db
        ...
        /* -----------------------------------------------------
           ...
           ----------------------------------------------------- */
        if (job.transportManagerId !==
            transportManagerId) {
            return res.status(403).json({
                error: 'You are not assigned as the transport manager for this job.',
            });
        }
        /* -----------------------------------------------------
           Validate job status
        ...
        if (job.currentStatus !== 'going_to_lab') {
            return res.status(400).json({
                error: `Items cannot be received at lab when job status is ` +
                    `'${job.currentStatus}'.`,
            });
        }
        /* -----------------------------------------------------
           ...
           ----------------------------------------------------- */
        const invalidItems = existingItems.filter((item) => item.currentStatus !==
            'pending_lab_receipt');
        if (invalidItems.length > 0) {
            return res.status(400).json({
                error: 'All selected items must be in pending_lab_receipt status.',
        ...
        const result = await db.transaction(async (tx) => {
            ...
            for (const item of existingItems) {
                const updatedItem = await updateJobItemWithStatusTransition(item.id, item.currentStatus, 'received_at_lab', async (transaction) => {
                    const [updated] = await transaction
                        .update(jobItems)
                        .set({
                        currentStatus: 'received_at_lab',
                        onsiteRepairAuthorized: false,
                        inlabRepairAuthorized: true,
                        updatedAt: new Date(),
                    })
                        .where(eq(jobItems.id, item.id))
                        .returning();
                    if (!updated) {
                        throw new Error(`Failed to update job item ${item.id}`);
                    }
                    return updated;
                }, transportManagerId, comment?.trim() ||
                    'Item received at the lab.', tx);
                updatedItems.push(updatedItem);
            }
            /* -----------------------------------------------------
               3. Job-level comment
        ...
        return res.status(500).json({
            error: 'Internal server error while receiving item at lab',
        });
    }
};
export const getJobsreceivedAtLab = async (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            res.status(401).json(new Error('Authorised User Not found'));
...
const isActiveRepairManager = async (userId) => {
    const [manager] = await db
        ...
        .innerJoin(userRoles, eq(userRoles.userId, users.id))
        .innerJoin(roles, eq(userRoles.roleId, roles.id))
        .where(and(eq(users.id, userId), eq(users.userType, 'employee'), eq(users.isActive, true), eq(roles.name, 'repair_manager')))
        .limit(1);
    return Boolean(manager);
};
export const getRepairManagers = async (req, res) => {
    try {
        const managers = await db
            ...
            .where(and(eq(users.userType, 'employee'), eq(users.isActive, true), eq(roles.name, 'repair_manager')))
        ...
        console.error('Get repair managers error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
};
export const assignRepairManager = async (req, res) => {
    try {
        const { jobId, repairManagerId, comment, } = req.body;
        const transportManagerId = req.user?.userId;
        if (!transportManagerId) {
            return res.status(401).json({
                error: 'Authenticated user not found',
        ...
        if (job.currentStatus !== 'going_to_lab' &&
            ...
            return res.status(400).json({
                error: `Cannot assign Repair Team Manager when job is ` +
                    `'${job.currentStatus}'.`,
            });
        }
        /* -----------------------------------------------------
           ...
           ----------------------------------------------------- */
        const repairManagerActive = await isActiveRepairManager(repairManagerId);
        if (!repairManagerActive) {
            return res.status(400).json({
                error: 'Assigned user must be an active employee with the repair_manager role.',
            });
        }
        /* -----------------------------------------------------
        ...
        const labItems = await db
            .select()
            .from(jobItems)
            .where(and(eq(jobItems.jobId, jobId), eq(jobItems.currentStatus, 'received_at_lab')));
        if (labItems.length === 0) {
            return res.status(400).json({
                error: 'No job items are currently received at the lab and ready for Repair Team Manager assignment.',
            });
        }
        /* -----------------------------------------------------
        ...
        const updatedJob = await db.transaction(async (tx) => {
            ...
            for (const item of labItems) {
                await updateJobItemWithStatusTransition(item.id, item.currentStatus, 'assigned_to_repair_manager', async (transaction) => {
                    const [updatedItem] = await transaction
                        .update(jobItems)
                        .set({
                        currentStatus: 'assigned_to_repair_manager',
                        updatedAt: new Date(),
                    })
                        .where(eq(jobItems.id, item.id))
                        .returning();
                    return updatedItem;
                }, transportManagerId, comment ||
                    'Item assigned to Repair Team Manager', tx);
            }
            /* -----------------------------------------------
        ...
        return res.status(200).json({
            message: 'Repair Team Manager assigned successfully.',
            job: updatedJob,
            assignedItemCount: labItems.length,
        });
    }
    catch (error) {
        console.error('Assign repair manager error:', error);
        return res.status(500).json({
            error: 'Internal server error',
        });
    }
...
export const getPendingDeliveries = async (_req, res) => {
    try {
        const candidateRows = await db
            ...
            .from(jobs)
            .innerJoin(jobItems, eq(jobItems.jobId, jobs.id))
            .where(and(eq(jobs.currentStatus, 'in_progress'), eq(jobItems.currentStatus, 'ready_for_delivery')))
        ...
        for (const row of candidateRows) {
            const existing = jobMap.get(row.job.id);
            if (existing) {
                existing.items.push(row.jobItem);
            }
            else {
                jobMap.set(row.job.id, {
                    job: row.job,
                    items: [row.jobItem],
                });
            }
        ...
        for (const entry of jobMap.values()) {
            const allItems = await getJobItems(entry.job.id);
            const allItemsReady = allItems.length > 0 &&
                allItems.every((item) => item.currentStatus ===
                    'ready_for_delivery');
            if (allItemsReady) {
                readyJobs.push({
                    job: entry.job,
...
export const assignDelivery = async (req, res) => {
    try {
        const { jobId, deliveryPersonId, comment, } = req.body;
        const transportManagerId = req.user?.userId;
        if (!transportManagerId) {
            return res.status(401).json({
                error: 'Authenticated user not found',
        ...
        if (job.currentStatus !==
            'in_progress') {
            return res.status(400).json({
                error: `Cannot assign delivery when job is '${job.currentStatus}'.`,
            });
        }
        /* -----------------------------------------------------
           Validate delivery person
           ----------------------------------------------------- */
        if (!(await isActiveDeliveryPerson(deliveryPersonId))) {
            return res.status(400).json({
                error: 'Delivery person must be an active employee with the transport_team_person role.',
            });
        }
        /* -----------------------------------------------------
           Get all job items
        ...
        if (items.length === 0) {
            return res.status(400).json({
                error: 'Cannot assign delivery because the job has no items.',
            });
        }
        /* -----------------------------------------------------
           Every item must be ready for delivery
           ----------------------------------------------------- */
        const allItemsReady = items.every((item) => item.currentStatus ===
            'ready_for_delivery');
        if (!allItemsReady) {
            return res.status(400).json({
                error: 'All job items must be ready_for_delivery before delivery can be assigned.',
            });
        }
        /* -----------------------------------------------------
        ...
        const updatedJob = await db.transaction(async (tx) => {
            const [result] = await tx
                .update(jobs)
                .set({
                assignedDeliveryTechId: deliveryPersonId,
                transportManagerId,
                updatedAt: new Date(),
            })
                .where(eq(jobs.id, jobId))
