...
import { jobs, jobItems, jobComments, jobStatusHistory, jobClosures, jobItemQuotes, deviceServiceCharges, jobItemQuoteLines, JobItemStatus, jobItemStatusHistory, JobSummaryStatus, users, customerProfiles, jobQuotes, invoices, warranties, } from '../../db/schema/index.js';
...
const createWarrantiesForClosedJob = async (
  tx: DbTransaction,
  jobId: string,
  warrantyStart: Date,
) => {
    ...
    for (const line of lines) {
      ...
      await tx
        ...
        .values({
          ...
          deviceSerialNumber:
            quoteItem.item.deviceSerialNumber,
  ...
  try {
    ...
    const result = await db.transaction(
      async (tx) => {
        ...
        await createWarrantiesForClosedJob(
          tx,
          jobId,
          closedAt,
        );
