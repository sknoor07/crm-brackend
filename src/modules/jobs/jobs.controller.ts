...
export const getJobWithItems = async (req: Request, res: Response<GetJobWithItemsResponse>) => {
  try {
    const rawData = await db.select()
      .from(jobs).leftJoin(jobItems, eq(jobs.id, jobItems.jobId));

    const jobMap = new Map<string, { job: JobSchema; jobItems: JobItemSchema[]; }>();

    for (const row of rawData) {
      if (!jobMap.has(row.jobs.id)) {
        jobMap.set(row.jobs.id, { job: row.jobs, jobItems: [], });
      }
      const existingJob = jobMap.get(row.jobs.id);

      if (existingJob && row.job_items) {
        existingJob.jobItems.push(row.job_items);
      }
    }
    const result = Array.from(jobMap.values());
    res.status(200).json({ message: "job Fetched successfully", result });
  } catch {
    res.status(500).json({ message: "Cannot get job and job Item Details",result: []
 })
  }
}
...
//     assignedRepairPersonId: jobItems.assignedRepairPersonId,
//     requestedComponents: jobItems.requestedComponents,
//     diagnosisNotes: jobItems.diagnosisNotes,
//     repairNotes: jobItems.repairNotes,
//     isWarrantyClaim: jobItems.isWarrantyClaim,
