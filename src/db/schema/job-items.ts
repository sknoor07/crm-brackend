...
export const jobItems = pgTable('job_items', {
    ...
    currentStatus: jobItemStatus('current_status')
        .notNull()
        .default('pending_cs_verification'),

    /**
     * Repair Team Manager assigns an individual item
     * to a repair person.
     */
    assignedRepairPersonId: uuid(
        'assigned_repair_person_id',
    ).references(() => users.id),
    ...
    isFinalQuoteApproved: boolean(
        'is_final_quote_approved',
    ).default(false),

    requestedComponents: text(
    ...
    originalJobItemId: uuid(
        'original_job_item_id',
    ),
